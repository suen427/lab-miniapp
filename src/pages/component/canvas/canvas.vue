    ...
    start(newVal, oldVal, owner, ins) {
      ...
      for (let i = 0; i < layer; i++) {
        ...
        for (let j = 0; j < ballInlayer; j++) {
          ...
          const x = radius * cos + canvasWidth / 2
          const y = radius * sin + canvasHeight / 2
          const vx = speed * cos
          const vy = speed * sin
  ...
  <view>
    ...
    <view class="page-body">
      <view class="page-body-wrapper">
        <!-- #ifdef APP-PLUS || H5 -->
        <canvas
          canvas-id="canvas" class="canvas" :start="startStatus" :change:start="animate.start"
          :data-width="canvasWidth" :data-height="canvasWidth"
        />
        <!-- #endif -->
        <!-- #ifndef APP-PLUS || H5 -->
